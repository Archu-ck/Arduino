...
#define PASSWORD_ADDR 500 // EEPROM address for password
...
void checkAndRingBell() {
  ...
  for (int i = 0; i < numEvents; i++) {
    if (now.hour() == schedule[i].hour && now.minute() == schedule[i].minute && now.second() == 0) {
      ringBell(schedule[i].rings);
      delay(60000); // wait to avoid repeating in same minute
    }
  }
}
...
String generateHTMLPage() {
  ...
  page += "<div class='card'>";
  DateTime now = rtc.now();
  page += "<div><strong>Current Time:</strong> " + String(now.hour()) + ":" + (now.minute() < 10 ? "0" : "") + String(now.minute()) + "</div>";
...
void savePasswordToEEPROM() {
  ...
  EEPROM.write(PASSWORD_ADDR + adminPassword.length(), '\0'); // null terminator
  EEPROM.commit();
}
